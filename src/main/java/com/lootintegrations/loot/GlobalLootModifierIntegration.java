...
public class GlobalLootModifierIntegration
{
    ...
    public void doApply(final List<ItemStack> generatedLoot, final LootContext context, final LootTable lootTable)
    {
        ...
        try
        {
            ...
            extraItems = ((Registry<LootTable>)context.getLevel().getServer().reloadableRegistries().lookup().lookup(Registries.LOOT_TABLE).get()).get(lootTableId).get().value().getRandomItems(context);
        }
        catch (Exception e)
        {
