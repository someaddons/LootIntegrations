...
public class Configuration
{
    ...
    public void load()
    {
        final Path configPath = FabricLoader.getInstance().getConfigDir().normalize().resolve(LootintegrationsMod.MODID + ".json");
        final File config = configPath.toFile();

        if (!config.exists())
        {
            LootintegrationsMod.LOGGER.warn("Config not found, recreating default");
            save();
        }
        else
        {
            ...
            {
                LootintegrationsMod.LOGGER.error("Could not read config from:" + configPath, e);
            }
        }
    }
    ...
    public void save()
    {
        ...
        {
            LootintegrationsMod.LOGGER.error("Could not write config to:" + configPath, e);
        }
    }
